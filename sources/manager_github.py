...
class GitHubManager:
    ...
    def _get_author() -> Actor:
        ...
        if EM.COMMIT_BY_ME:
            return Actor(
                EM.COMMIT_USERNAME or GitHubManager.USER.login,
                EM.COMMIT_EMAIL or GitHubManager.USER.email,
            )
        else:
            return Actor(
    ...
    def update_chart(name: str, path: str) -> str:
        ...
        if not EM.DEBUG_RUN:
            ...
            chart_path = f"https://raw.githubusercontent.com/" f"{GitHubManager._REMOTE_NAME}/{branch}/{path}"
    ...
    def commit_update():
        ...
        if EM.COMMIT_SINGLE:
            ...
            refspec = f"{GitHubManager._SINGLE_COMMIT_BRANCH}:" f"{GitHubManager.branch(EM.PUSH_BRANCH_NAME)}"
    ...
    def get_repository_data(repo_name: str):
        ...
        try:
            github = Github(EM.GH_TOKEN)
            repo_data = github.get_repo(f"{GitHubManager.USER.login}/{repo_name}").raw_data
